...
[![版本](https://img.shields.io/badge/version-1.1.0rc2-blue.svg)](#)
...
- **危险执行检测** — 识别破坏性 Shell、下载后执行、不安全权限修改、解码后执行
  和反弹 Shell 等模式。
- **Prompt Injection 检测** — 检查 Tool Result 中的英文、中文、结构化任务重定向、
  POT（Prompt-Override Tool）、间接注入和针对特定 Agent 的攻击模式。
- **记忆污染检测** — 检测上下文操纵、记忆检索攻击、指令替换尝试和 Tool Result
  中的伪造历史上下文。
- **权限提升检测** — 检测 sudo/su 命令、Docker 特权容器、setuid 修改、容器逃逸
  尝试和系统配置篡改。
- **密钥泄露检测** — 识别 20+ 种凭证模式，包括 AWS 密钥、OpenAI API Key、
  GitHub Token、私钥等在 Tool Result 和参数中的泄露。
- **代码注入检测** — 捕获代码写入-执行链、动态代码评估（eval/exec）和
  下载-执行模式。
- **序列检测** — 识别注入后访问敏感资源或执行危险命令、敏感读取后向外部
  传输数据，以及代码注入链。
...
ZeroADR 在 **ASB**（Agent Security Benchmark）上进行了评估，这是一个综合性的
安全基准测试，针对 5 种攻击类型、100 个攻击场景和 100 个干净基线任务。该基准
测量攻击成功率（ASR）、防护率和误报率，分为三个臂：baseline（无防护）、
rules-only（仅规则）和 hybrid（规则 + LLM 审查）。
...
| 攻击场景 | 100（每种类型 20 个） |
| 干净任务 | 100 |
| 总样本数 | 每臂 200 个 × 3 臂 = 600 |
| Agent 模型 | `deepseek-v4-pro` |
| Reviewer 模型 | `deepseek-v4-flash` |
| Review Prompt | `tool-result-review-v0.2` |
| 并发 Workers | 4 |

#### 结果总结

| Arm | ASR | 防护率 | 干净误报 | Block 次数 | 任务成功率 | P50 延迟 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
**主要成就：**
- **ASR 从 73% 降至 0%**，覆盖所有攻击类型
- **Rules-only 模式零误报**
- **Hybrid 模式误报率 1%**
- **全部 5 种攻击类型均被拦截：** DPI、OPI、Memory Poisoning、Mixed、POT

#### 按攻击类型分拆（Hybrid）

| 攻击类型 | ASR | 样本数 |
| --- | ---: | ---: |
| DPI（直接 Prompt 注入） | **0%** | 0/20 |
| OPI（间接 Prompt 注入） | **0%** | 0/20 |
| Memory Poisoning（记忆污染） | **0%** | 0/20 |
| Mixed（混合攻击） | **0%** | 0/20 |
| POT（Prompt-Override Tool） | **0%** | 0/20 |

实现 0% ASR 的关键是**工具调用层面拦截**：恶意攻击者工具在执行前被拦截，
防止攻击目标字符串出现在对话中。这适用于所有攻击类型。
