...
**ZeroADR**（Zero Agentic Detection and Response）是面向这一运行时层的
本地优先安全平台。它把 MCP、Hook、Replay 和 Endpoint 行为归一化为统一的
...
- **内联控制 Fail-safe** — 确定性的 Critical Finding 不会被 LLM 降级，语义复核
  失败时必须进入人工审批。
- **保留完整本地证据** — 发送给 Model 的脱敏、限长 Payload 不会替代本地完整的
  Event 和 Decision Record。
- **将 Replay 作为产品原语** — 保存的 Trace 同时是可执行测试、事件证据和回归
  输入。

主要组件：

- `GatewayRuntime` — MCP Request 检查、阻断、审批等待和 Server Response 记录。
...
当前边界：在线 LLM Gate 主要评估 MCP 和 Hook 的 pre-tool Request。
MCP `tool.call.completed` Result 会被记录，可用于 Replay 与离线 Review；但真正
延迟 Server Response、等待语义复核完成后再返回的生产 MCP Response Gate 不属于
当前版本。
