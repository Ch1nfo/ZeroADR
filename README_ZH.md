...
[![版本](https://img.shields.io/badge/version-1.2.0rc1-blue.svg)](#当前版本)
...
- **多阶段 Runtime Gate** — 可选地在后续运行步骤前审查 Agent Input、发现的
  Tool Metadata、Tool Request 和 Tool Result。
- **在线 Session Guard** — 在有界状态中传播 Session Compromise，使已检测到的
  Injection 能阻断后续写入、执行、敏感读取或外传，而不只依赖事后 Replay。
...
- 面向 Agent Input、Tool Metadata、Tool Request 的 Stage-specific Hybrid Review，
  以及 `tool-result-review-v0.2` Result Review
...
v1.2 的所有新 Gate 均为显式启用；配置缺失时保持 v1.1 行为。MCP 可同步执行
Metadata、Request 和 Result Gate。Agent Input Gate 需要 Generic/Claude Code Input
Hook，因为仅部署 MCP 无法看到初始 User Prompt。Hook post-tool、Endpoint 和 Replay
路径仍保持观测性质。
...
Hybrid 相比 Baseline 将 ASR 降低 **63 个百分点**，相对降幅 90%；100 个 Clean
Control 中有 4 个误报 Case。ASR 与 Clean FPR 目标通过，但任务成功率比 Baseline
低 11.5 个百分点，超出效用目标 1.5 个百分点。因此该结果标记为“安全目标通过、
效用目标未通过”，不描述为全部验收成功。
...
Hybrid 有 59 个 Case 在执行前阻断，30 个 Case 在 Result-stage 或执行后阻断；6 个
成功攻击完全漏检，1 个攻击在出现阻断后仍被官方 Evaluator 判定成功。Stage Action
分别为 Input 40 Block / 1 Approval、Metadata 5 / 1、Pre-tool 17 / 9、Result 32 / 7。
Hybrid 使用 508 次 Agent 调用和 878 次 Reviewer 调用，Reviewer p95 为 9.410s。
Memory Poisoning 以成功检索污染记忆为前提。

正式 Cache 包含 600 个终态 Case。连续两次纯 Cache 复跑均为零 Case Run、零模型
调用，并复现 Analysis SHA-256
`42d8cc195623a19fd7304018353af6293d9799d35fa2e2135babef92f379a8e5`。
完整方法和 Hash 见 [ASB 结果报告](evaluation/asb/RESULTS.md)。
...
`1.2.0rc1` 在生产 Tool Result Gate 之外新增可选的 Agent Input、Tool Metadata、
Tool Request 和在线 Session Guard，并提供 Stage-specific 脱敏审计记录。官方 ASB
Adapter 复用同一核心 Coordinator，且不向防护逻辑暴露 Benchmark Label。缺少新
Gate 配置时保持 v1.1 行为。Linux BCC 实机验证已经通过。版本历史统一记录在
...
- [多阶段 Runtime Gate](docs/runtime-gates.md)
